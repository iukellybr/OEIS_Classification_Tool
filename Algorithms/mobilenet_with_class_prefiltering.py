            ...
            for file in os.listdir(class_path):
                if file.endswith(".png"):
                    ...
                    try:
                        ...
                        with torch.no_grad():
                            features = model.features(image.unsqueeze(0).to(device))
                            embedding = features.mean(dim=[2, 3]).cpu().numpy()[0]
                        class_embeddings.append(embedding)
                    except Exception as e:
                        logger.warning(f"Failed to process reference image {image_path}: {e}")
            if class_embeddings:
                reference_classes[class_name] = np.array(class_embeddings)
    return reference_classes

# Identify sequences that are within known classes
def exclude_known_classes(embeddings, sequence_ids, reference_classes, threshold=2.5):
    keep_indices = []
    distance_debug = []
    excluded = 0
    ...
    for i, emb in enumerate(embeddings):
        ...
        for class_name, class_embs in reference_classes.items():
            dists = euclidean_distances([emb], class_embs)
            dist = np.min(dists)
            if dist < min_dist:
                min_dist = dist
                best_class = class_name
        
        distance_debug.append(min_dist)
